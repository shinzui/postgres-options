...
userInfoToptions UserInfo {..} =
  mempty {user = return $ BSC.unpack uiUsername}
    <> if BS.null uiPassword
      then mempty
      else mempty {password = return $ BSC.unpack uiPassword}

authorityToOptions :: Authority -> Options
authorityToOptions Authority {..} =
  maybe mempty userInfoToptions authorityUserInfo
...
parseInt msg v =
  maybe (Left (msg <> " value of: " <> v <> " is not a number")) Right $
    readMaybe v
...
uriToptions URI {..} = case schemeBS uriScheme of
  "postgresql" -> do
    ...
    return $
      maybe mempty authorityToOptions uriAuthority
        <> pathToptions uriPath
        <> queryParts
  x -> Left $ "Wrong protocol. Expected \"postgresql\" but got: " ++ show x

parseURIStr :: String -> Either String (URIRef Absolute)
parseURIStr = left show . parseURI strictURIParserOptions . BSC.pack
  where
    left f = \case
      Left x -> Left $ f x
      Right x -> Right x
...
parseKeywords x = fmap mconcat . mapM (uncurry keywordToptions <=< toTuple . splitOn "=") $ words x
  where
